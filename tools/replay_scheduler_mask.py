...
SCHEDULER_OUTPUT_QOS = QoSProfile(
...
def mask_scheduler_state(message, disabled_modalities):
    ...
    for index, name in enumerate(output.modality_names):
        ...
        reason = output.reasons[index]
        output.reasons[index] = ",".join(
            item for item in (reason, "replay_ablation_disabled") if item
        )
    return output
...
class SchedulerMask(Node):
    ...
    def _message(self, message):
        self.publisher.publish(
            mask_scheduler_state(message, self.disabled_modalities)
        )
...
def main():
    ...
    parser.add_argument(
        "--input", default="/replay/reliability/scheduler_state_input"
    )
    parser.add_argument("--output", default="/reliability/scheduler_state")
    parser.add_argument(
        "--disable", action="append", choices=("lidar", "gnss"), default=[]
    )
