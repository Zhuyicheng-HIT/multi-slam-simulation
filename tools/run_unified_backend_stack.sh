...
LIDAR_PREDICTION_GATE_ENABLED=${LIDAR_PREDICTION_GATE_ENABLED:-true}
...
setsid env \
  ...
  enable_lidar_calibration_motion:="$ENABLE_LIDAR_CALIBRATION_MOTION" \
  reliability_mode:="$RELIABILITY_MODE" \
  lidar_factor_score_mode:="$LIDAR_FACTOR_SCORE_MODE" \
  lidar_admission_mode:="$LIDAR_ADMISSION_MODE" \
  lidar_paper_activation_threshold:="$LIDAR_PAPER_ACTIVATION_THRESHOLD" \
  lidar_prediction_gate_enabled:="$LIDAR_PREDICTION_GATE_ENABLED" \
