...
class UnifiedBackendNode(Node):
    ...
    def _visual_factor(self, previous_index, current_index,
        ...
        if current is None or arrival is None:
            ...
            return
        if time.monotonic() - arrival > self.visual_max_arrival_age_s:
        ...
        if (
            self.reliability_mode == "dynamic"
            and not self._score_is_fresh("vision", time.monotonic())
        ):
    ...
    def log_final_summary(self):
        ...
        print(
            ...
            f"lidar_factors={self.counts['lidar_factors']};"
            f"gnss_factors={self.counts['gnss_factors']};"
            f"gnss_jump_rejected={self.counts['gnss_jump_rejected']};"
            f"flow_factor_attempts={self.counts['flow_factor_attempts']};"
