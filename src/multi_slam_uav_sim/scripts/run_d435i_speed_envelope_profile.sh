#!/usr/bin/env bash
set -eo pipefail

SCRIPT_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
PKG_SHARE=$(cd "$SCRIPT_DIR/.." && pwd)
WS_INSTALL=$(cd "$PKG_SHARE/../../.." && pwd)
WS_ROOT=$(cd "$WS_INSTALL/.." && pwd)
source /opt/ros/humble/setup.bash
source "$WS_INSTALL/setup.bash"
set -u

SPEED_TEST_PROFILE=${SPEED_TEST_PROFILE:-horizontal}
case "$SPEED_TEST_PROFILE" in
  horizontal|vertical|yaw|combined|small_rectangle|loop_return) ;;
  *)
    printf 'Unknown SPEED_TEST_PROFILE=%s\n' "$SPEED_TEST_PROFILE" >&2
    exit 2
    ;;
esac

HORIZONTAL_SPEED_MPS=${HORIZONTAL_SPEED_MPS:-0.10}
VERTICAL_SPEED_MPS=${VERTICAL_SPEED_MPS:-0.10}
YAW_RATE_DEG_S=${YAW_RATE_DEG_S:-8.0}
STRAIGHT_DISTANCE_M=${STRAIGHT_DISTANCE_M:-3.0}
VERTICAL_EXCURSION_M=${VERTICAL_EXCURSION_M:-0.70}
RECTANGLE_X_M=${RECTANGLE_X_M:-0.75}
RECTANGLE_Y_M=${RECTANGLE_Y_M:-0.50}
PRE_OBSERVATION_S=${PRE_OBSERVATION_S:-5.0}
POST_OBSERVATION_S=${POST_OBSERVATION_S:-5.0}
MOTION_HOLD_S=${MOTION_HOLD_S:-3.0}
INITIAL_TAKEOFF_ALT=${INITIAL_TAKEOFF_ALT:-0.25}
FLIGHT_ALTITUDE_M=${FLIGHT_ALTITUDE_M:-0.50}
normalize_double() {
  printf '%.6f' "$1"
}
HORIZONTAL_SPEED_MPS=$(normalize_double "$HORIZONTAL_SPEED_MPS")
VERTICAL_SPEED_MPS=$(normalize_double "$VERTICAL_SPEED_MPS")
YAW_RATE_DEG_S=$(normalize_double "$YAW_RATE_DEG_S")
STRAIGHT_DISTANCE_M=$(normalize_double "$STRAIGHT_DISTANCE_M")
VERTICAL_EXCURSION_M=$(normalize_double "$VERTICAL_EXCURSION_M")
RECTANGLE_X_M=$(normalize_double "$RECTANGLE_X_M")
RECTANGLE_Y_M=$(normalize_double "$RECTANGLE_Y_M")
PRE_OBSERVATION_S=$(normalize_double "$PRE_OBSERVATION_S")
POST_OBSERVATION_S=$(normalize_double "$POST_OBSERVATION_S")
MOTION_HOLD_S=$(normalize_double "$MOTION_HOLD_S")
INITIAL_TAKEOFF_ALT=$(normalize_double "$INITIAL_TAKEOFF_ALT")
FLIGHT_ALTITUDE_M=$(normalize_double "$FLIGHT_ALTITUDE_M")

ACTIVE_FILE=${ACTIVE_FILE:-/tmp/multi_slam_d435i_visual_slam.active}
if [[ ! -f "$ACTIVE_FILE" ]]; then
  printf 'Start run_d435i_visual_slam_headless.sh first.\n' >&2
  exit 2
fi
read -r wrapper_pid headless_run_dir <"$ACTIVE_FILE"
if ! kill -0 "$wrapper_pid" 2>/dev/null; then
  printf 'Recorded headless wrapper is not running: %s\n' "$wrapper_pid" >&2
  exit 2
fi

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
target_label=${HORIZONTAL_SPEED_MPS}mps
if [[ "$SPEED_TEST_PROFILE" == "yaw" ]]; then
  target_label=${YAW_RATE_DEG_S}dps
elif [[ "$SPEED_TEST_PROFILE" == "vertical" ]]; then
  target_label=${VERTICAL_SPEED_MPS}mps
elif [[ "$SPEED_TEST_PROFILE" == "combined" ]]; then
  target_label=${HORIZONTAL_SPEED_MPS}mps_${YAW_RATE_DEG_S}dps
fi
OUTPUT_DIR=${OUTPUT_DIR:-$WS_ROOT/logs/d435i_visual_slam/speed_envelope/${TIMESTAMP}_${SPEED_TEST_PROFILE}_${target_label}}
mkdir -p "$OUTPUT_DIR"
PROFILER_LOG="$OUTPUT_DIR/profiler.log"
MOTION_LOG="$OUTPUT_DIR/motion.log"
rtabmap_log="$headless_run_dir/rtabmap.log"
profiler_pid=""

cleanup_profiler() {
  if [[ -n "$profiler_pid" ]] && kill -0 "$profiler_pid" 2>/dev/null; then
    kill -INT -- "-$profiler_pid" 2>/dev/null || kill -INT "$profiler_pid" 2>/dev/null || true
    for _ in {1..50}; do
      if ! kill -0 "$profiler_pid" 2>/dev/null; then
        wait "$profiler_pid" 2>/dev/null || true
        return 0
      fi
      sleep 0.2
    done
    kill -TERM -- "-$profiler_pid" 2>/dev/null || kill -TERM "$profiler_pid" 2>/dev/null || true
    wait "$profiler_pid" 2>/dev/null || true
  fi
  return 0
}
trap cleanup_profiler EXIT INT TERM

if [[ ! -f "$rtabmap_log" ]]; then
  printf 'Active RTAB-Map log is missing: %s\n' "$rtabmap_log" >&2
  exit 2
fi

cat >"$OUTPUT_DIR/run_context.txt" <<EOF
speed_test_profile=$SPEED_TEST_PROFILE
output_dir=$OUTPUT_DIR
headless_run_dir=$headless_run_dir
git_commit=$(git -C "$WS_ROOT" rev-parse HEAD)
git_branch=$(git -C "$WS_ROOT" branch --show-current)
rtabmap_profile=${RTABMAP_PROFILE:-unknown}
horizontal_speed_mps=$HORIZONTAL_SPEED_MPS
vertical_speed_mps=$VERTICAL_SPEED_MPS
yaw_rate_deg_s=$YAW_RATE_DEG_S
straight_distance_m=$STRAIGHT_DISTANCE_M
vertical_excursion_m=$VERTICAL_EXCURSION_M
rectangle_x_m=$RECTANGLE_X_M
rectangle_y_m=$RECTANGLE_Y_M
bridge_impl=cpp
depth_encoding=16UC1
resolution=640x480
exact_sync=true
qos_reliability=reliable
qos_depth=1
navigation_source=gps_guided
rtab_controls_flight=false
EOF

node_list=$(ros2 node list 2>/dev/null || true)
odom_node=$(printf '%s\n' "$node_list" | awk '/\/rgbd_odometry$/ {print; exit}')
rtab_node=$(printf '%s\n' "$node_list" | awk '/\/rtabmap$/ {print; exit}')
if [[ -z "$odom_node" || -z "$rtab_node" ]]; then
  printf 'Could not resolve active RTAB-Map nodes.\n' >&2
  exit 1
fi
{
  printf 'capture:\n'
  printf '  profile: %s\n' "${RTABMAP_PROFILE:-unknown}"
  printf '  speed_test_profile: %s\n' "$SPEED_TEST_PROFILE"
  printf '  odometry_node: %s\n' "$odom_node"
  printf '  mapping_node: %s\n' "$rtab_node"
  printf 'nodes:\n'
  printf '  odometry:\n    name: %s\n    dump:\n' "$odom_node"
  ros2 param dump "$odom_node" | sed 's/^/      /'
  printf '  mapping:\n    name: %s\n    dump:\n' "$rtab_node"
  ros2 param dump "$rtab_node" | sed 's/^/      /'
} >"$OUTPUT_DIR/parameters.yaml"
{
  printf 'node\tparameter\tros2_param_get\n'
  for spec in \
    "$odom_node|Vis/FeatureType" \
    "$odom_node|Vis/MinInliers" \
    "$odom_node|Odom/ResetCountdown" \
    "$rtab_node|Kp/DetectorStrategy" \
    "$rtab_node|Vis/FeatureType" \
    "$rtab_node|Mem/UseOdomFeatures" \
    "$rtab_node|Rtabmap/LoopThr"; do
    node=${spec%%|*}
    parameter=${spec#*|}
    value=$(ros2 param get "$node" "$parameter" 2>&1 || true)
    printf '%s\t%s\t%s\n' "$node" "$parameter" "$value"
  done
} >"$OUTPUT_DIR/parameter_checks.tsv"
grep -E \
  'Odometry: (Update|Ignored) parameter|Update RTAB-Map parameter|Mem/UseOdomFeatures|Vis/FeatureType|Kp/DetectorStrategy' \
  "$rtabmap_log" >"$OUTPUT_DIR/internal_parameter_evidence.log" || true

setsid ros2 run multi_slam_uav_sim d435i_speed_envelope_profiler --ros-args \
  -p use_sim_time:=true \
  -p output_dir:="$OUTPUT_DIR" \
  -p profile:="speed_${SPEED_TEST_PROFILE}" \
  -p speed_test_profile:="$SPEED_TEST_PROFILE" \
  -p commanded_horizontal_speed_mps:="$HORIZONTAL_SPEED_MPS" \
  -p commanded_vertical_speed_mps:="$VERTICAL_SPEED_MPS" \
  -p commanded_yaw_rate_deg_s:="$YAW_RATE_DEG_S" \
  -p duration_s:=0.0 \
  -p rtabmap_log_path:="$rtabmap_log" \
  >"$PROFILER_LOG" 2>&1 &
profiler_pid=$!

for _ in {1..60}; do
  if grep -q 'Speed profiler:' "$PROFILER_LOG" 2>/dev/null; then break; fi
  if ! kill -0 "$profiler_pid" 2>/dev/null; then
    cat "$PROFILER_LOG" >&2
    exit 1
  fi
  sleep 0.25
done
if ! grep -q 'Speed profiler:' "$PROFILER_LOG" 2>/dev/null; then
  printf 'Speed profiler did not become ready.\n' >&2
  exit 1
fi
if ! timeout 10s ros2 topic echo /front/d435i/transport/frame_tracking --once >/dev/null 2>&1; then
  printf 'Bridge tracking diagnostic has no data.\n' >&2
  exit 1
fi

set +e
ros2 run multi_slam_uav_sim d435i_speed_envelope_motion --ros-args \
  -p use_sim_time:=true \
  -p speed_test_profile:="$SPEED_TEST_PROFILE" \
  -p navigation_source:=gps \
  -p takeoff_alt:="$INITIAL_TAKEOFF_ALT" \
  -p takeoff_min_alt_m:=0.15 \
  -p takeoff_free_climb_s:=14.0 \
  -p pre_observation_s:="$PRE_OBSERVATION_S" \
  -p post_observation_s:="$POST_OBSERVATION_S" \
  -p flight_altitude_m:="$FLIGHT_ALTITUDE_M" \
  -p horizontal_speed_mps:="$HORIZONTAL_SPEED_MPS" \
  -p vertical_speed_mps:="$VERTICAL_SPEED_MPS" \
  -p yaw_rate_deg_s:="$YAW_RATE_DEG_S" \
  -p straight_distance_m:="$STRAIGHT_DISTANCE_M" \
  -p vertical_excursion_m:="$VERTICAL_EXCURSION_M" \
  -p rectangle_x_m:="$RECTANGLE_X_M" \
  -p rectangle_y_m:="$RECTANGLE_Y_M" \
  -p motion_hold_s:="$MOTION_HOLD_S" \
  >"$MOTION_LOG" 2>&1
status=$?
set -e

sleep 2
cleanup_profiler
profiler_pid=""
trap - EXIT INT TERM

if [[ -f "$headless_run_dir/d435i_bridge_performance.csv" ]]; then
  cp "$headless_run_dir/d435i_bridge_performance.csv" "$OUTPUT_DIR/bridge_performance.csv"
fi
printf 'motion_exit_code=%s\n' "$status" >>"$OUTPUT_DIR/run_context.txt"
if [[ ! -f "$OUTPUT_DIR/summary.md" ]]; then
  printf 'Profiler did not produce summary.md.\n' >&2
  cat "$PROFILER_LOG" >&2
  exit 1
fi
printf 'Speed-envelope result: %s\n' "$OUTPUT_DIR"
cat "$OUTPUT_DIR/summary.md"
exit "$status"
