...
if [[ "${START_SITL:-1}" == "1" ]]; then
  ...
  # Synthetic clock is intentionally not enabled: Gazebo owns simulation time
  # and the JSON FDM timestamp must advance at the configured real-time rate.
...
if [[ "${RECTANGLE_FLOW_TEST:-0}" == "1" || "${AUTO_FLIGHT:-0}" == "1" ]]; then
  ...
  if [[ "${temporal_dynamic_filter_enabled:-false}" == "true" ]]; then
