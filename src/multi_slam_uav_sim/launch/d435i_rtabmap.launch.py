...
def _launch_setup(context):
    ...
    def topic(name, configured, fallback):
        override = LaunchConfiguration(name).perform(context).strip()
        return override or topics.get(configured, fallback)

    launch_arguments = {
        "rgb_topic": topic(
            "rgb_topic", "rgb", "/front/d435i/color/image_raw"),
        "depth_topic": topic(
            "depth_topic", "depth",
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            ...
            description="Optional RGB input override; empty uses profile YAML."),
        DeclareLaunchArgument(
            "depth_topic", default_value="",
            description="Optional depth input override; empty uses profile YAML."),
        DeclareLaunchArgument(
